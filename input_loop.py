...
from nocrosshair.features.aim_assist import AimAssistEngine, AimAssistPipeline, JitterEngine, RushEngine
...
class InputPipeline:
    ...
    def __init__(self, config: AppConfig, overlay: Optional[Any] = None):
        ...
        self.aa_pipeline = AimAssistPipeline(
            AimAssistEngine(config.aim_assist), JitterEngine()
        )
        self.rush_engine = RushEngine(
            pulse_ms=config.aim_assist.rush_pulse_ms,
            cooldown_ms=config.aim_assist.rush_cooldown_ms,
            deadzone=config.aim_assist.rush_deadzone
        )
...
def _is_ds4_controller(dev: Optional[InputDevice]) -> bool:
    if not dev:
        return False
    try:
        if getattr(dev.info, 'vendor', 0) == 0x054C:
            return True
...
class InputLoop:
        ...
        try:
            ...
            if self.config.remap_kbd_path and self.config.remap_kbd_path != path:
                try:
                    ...
                    self._kbd_fd = kbd.fd
                    grabbed_any = True
                    print(f"[InputLoop] KBD opened (no grab): {kbd.name} ({self.config.remap_kbd_path})")
            ...
            try:
                ...
                if has_keys and not has_abs and not has_rel:
                    ...
                    if e.KEY_A in key_caps and e.KEY_1 in key_caps:
                        self._devices.append(dev)
                        self._kbd_fd = dev.fd
                        print(f"[InputLoop] KBD auto-detected (no grab): {dev.name} ({dev_path})")
                        return
                dev.close()
            except (PermissionError, OSError):
                continue
    ...
    def _handle_global_event(self, event) -> bool:
        ...
        if event.type == e.EV_KEY and event.code == e.KEY_DELETE and event.value == 1:
            ...
            if self._disabled:
                ...
                for d in self._devices:
                    try:
                        # Only grab mouse devices, leave keyboard ungrabbed to keep original keys working
    ...
    def _handle_remap_key_event(self, event, default_prefix: str) -> None:
        ...
        norm_value = event.value  # agora é sempre 0 ou 1
    ...
    def _handle_controller_event(self, event, dev: Optional[InputDevice] = None) -> None:
        ...
        if event.type == e.EV_ABS:
            val = event.value
            code = event.code
            if is_ds4 and code in (e.ABS_X, e.ABS_Y, e.ABS_RX, e.ABS_RY):
